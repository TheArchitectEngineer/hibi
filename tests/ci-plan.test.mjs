...
test('runner image updates rerun tests but reuse builds with the same toolchain', (t) => {
  const root = mkdtempSync(join(tmpdir(), 'hibi-ci-image-'))
  t.after(() => rmSync(root, { recursive: true, force: true }))
  writeFileSync(join(root, '.gitignore'), '.cache/\nout/\n')
  writeFileSync(join(root, 'README.md'), 'fixture')
  mkdirSync(join(root, 'tests'))
  writeFileSync(
    join(root, 'tests', 'quick.test.mjs'),
    "import test from 'node:test'; test('quick', () => {})",
  )
  for (const file of [
    'out/main/index.js',
  ...
  execFileSync(
    'git',
    ['add', '.gitignore', 'README.md', 'tests/quick.test.mjs'],
    {
      cwd: root,
      stdio: 'pipe',
    },
  ...
  const npm = join(root, '.cache', 'fake-npm.mjs')
  mkdirSync(dirname(npm), { recursive: true })
  writeFileSync(npm, '')
  const ci = fileURLToPath(new URL('../scripts/ci.mjs', import.meta.url))
  const run = (ImageVersion) => {
    const env = {
      ...process.env,
      ImageVersion,
      npm_execpath: npm,
    ...
    return execFileSync(process.execPath, [ci], {
      cwd: root,
      encoding: 'utf8',
      env,
    })
  }
  const first = run('20260920.314.1')
  assert.match(first, /ci: full checks; build=true/)
  assert.match(first, /ok 1 - quick/)
  assert.match(
    run('20260907.300.1'),
    /ci: full checks; build=false; 1\/1 test files;[\s\S]*ok 1 - quick/,
  )
  assert.match(
    run('20260907.300.1'),
    /ci: incremental checks; build=false; 0\/1 test files/,
  )
})
