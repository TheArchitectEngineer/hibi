...
Choose **Open a folder…** in the sidebar, use **Open workspace…** in the command palette, or drop a folder onto Hibi. The command palette also lists recent workspaces, so you can search by folder name or path after dismissing the welcome screen. Opening a folder leaves your current note intact.
